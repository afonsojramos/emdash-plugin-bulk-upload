...
const ISO_DATE_PATTERN = /^(\d{4})-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01])(?:T.*)?$/;
...
export const DEFAULT_LABELS: BulkUploadLabels = {
  eyebrow: "",
  title: "Bulk upload",
  intro: "Create a reviewed draft for every file. Nothing is published automatically.",
  ...
  reload: "Try again",
  incomplete: "Complete the shared details and every file before creating drafts.",
...
export function resolveText(text: LocalizedText | undefined, lang: string, fallback = ""): string {
  if (text == null) return fallback;
  if (typeof text === "string") return text;
  return text[lang] ?? text[baseLanguage(lang)] ?? text.en ?? Object.values(text)[0] ?? fallback;
}
...
export function contentLabel(entry: LabeledEntry, labelKeys: string[] = ["name", "title"]): string {
