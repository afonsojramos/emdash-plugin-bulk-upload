...
describe("imageFieldValue", () => {
  ...
  it("uses the url as preview for remote providers", () => {
    const value = imageFieldValue({ ...base, provider: "r2", meta: { bucket: "media" } }, "Poster");
...
describe("resolveText", () => {
  ...
  it("falls back from a regional code to its base language", () => {
    ...
    assert.equal(resolveText({ pt: "Local", "pt-BR": "Localização" }, "pt-BR"), "Localização");
  });
...
describe("resolveLabels", () => {
  ...
  it("returns defaults for unknown languages", () => {
    assert.deepEqual(resolveLabels({ pt: { title: "x" } }, "vi"), DEFAULT_LABELS);
  });

  it("layers regional catalogs over the base language", () => {
    assert.equal(resolveLabels(undefined, "pt").hint, BUILT_IN_LANGUAGES.pt?.hint);
    assert.equal(resolveLabels(undefined, "pt-BR").hint, BUILT_IN_LANGUAGES["pt-BR"]?.hint);
    assert.notEqual(
